...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (inputArray.length) {
      ...
      function prepareUpload(event) {
        if (this.files && this.files[0]) {
          ...
          // Hide the add button container when an image is uploaded
          var addButtonContainer = document.querySelector(".add-btn");
          if (addButtonContainer) {
            addButtonContainer.style.display = "none"; // ✅ Hide only if exists
          } else {
            console.warn("⚠️ Warning: .add-btn not found in the DOM.");
          }
        }
      }
    }
  ...
  /* Cashback Filter Buttons */

  function setupCashbackFilters() {
    const buttons = document.querySelectorAll(".category-btn");
    const adCards = document.querySelectorAll("[data-category]");

    // REMOVE OLD EVENT LISTENERS (Swup Fix)
    buttons.forEach((button) => {
      button.replaceWith(button.cloneNode(true)); // Cloning removes old listeners
    });

    // Re-select buttons after cloning
    const newButtons = document.querySelectorAll(".category-btn");
    ...
    newButtons.forEach((button) => {
      button.addEventListener("click", function () {
        ...
        // Show/hide ad cards based on category
  ...
  // Ensure script runs on first page load
  document.addEventListener("DOMContentLoaded", () => {
    setupCashbackFilters();
  });

  // Ensure script runs after Swup replaces content
  ...
  let mapInstance; // Store the Leaflet map instance globally
  ...
  function initMap() {
    ...
    // ✅ Fix: Properly remove old map instance if it exists
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        function (position) {
          ...
          mapInstance = L.map("map").setView(userLocation, 15);
        ...
        function () {
          console.error("❌ Geolocation error! User did not allow location.");
        }
      );
    } else {
  ...
  // Swup event listener (ensure map reloads)
  document.addEventListener("swup:contentReplaced", function () {
    if (document.getElementById("map")) {
      // ✅ If Leaflet.js is already loaded, do NOT reload it again
      if (window.L) {
        initMap();
      } else {
  ...
  function initChart() {
    ...
    if (!chartContainer) {
      console.warn("⚠️ No chart container found!");
      return;
    }

    // 🔥 Fix: Destroy old chart instance if it exists
    if (chartInstance) {
    ...
    chartInstance = new ApexCharts(chartContainer, {
      ...
      plotOptions: {
        ...
        /* pie: {
      donut: { labels: { show: true, total: { show: true, label: "Total" } } },
    }, */
      },
  ...
  document.addEventListener("swup:contentReplaced", function () {
    ...
    // Run cashback filter script separately
    setupCashbackFilters();

    // Ensure all scripts are reloaded correctly
    init_template();
  });
